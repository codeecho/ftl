...
export default class BattleField extends Component{
    ...
    renderPlayer(player){
        const {name, health, maxHealth, attack, defense, element, weapon,
            ...
            statuses, relics, magicPoints, maxMagicPoints
        } = player;
                                    ...
                                    <tbody>
                                        {this.state.event.targets.map(target => {
                                            return (<tr>
                                                ...
                                                <td>
                                                    ...
                                                    {this.state.event.attacker.spells.filter(spell => spell.target === ENEMY).map(spell => <Button disabled={spell.cost > this.state.event.attacker.magicPoints} onClick={() => this.castSpell(this.state.event.attacker, target, spell)}>{spell.name} ({spell.cost})</Button>)}
                                                </td>
                                            </tr>)
                                        })}
                                        {this.state.event.allies.map(target => {
                                            return (<tr>
                                                <td>{target.name}</td>                                    
                                                <td>
                                                    {this.state.event.attacker.spells.filter(spell => spell.target === ALLIES).map(spell => <Button disabled={spell.cost > this.state.event.attacker.magicPoints} onClick={() => this.castSpell(this.state.event.attacker, target, spell)}>{spell.name} ({spell.cost})</Button>)}
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
